...
const CollabrationRecord = () => {
  return <div className="main - content">Collabration Record</div>;
};
const CreateIPv6 = () => {
  return <div className="main - content">Create IPv6</div>;
};
...
const Toolbar: React.FC<{
  ...
  const [IPv6show, setIPv6Show] = useState(false);
  ...
  const handleOpen = (al: boolean) => {
    setShow(al);
  };
  const handleIPv6Open = (al: boolean) => {
    setIPv6Show(al);
  };
  return (
    <div
    ...
    >
      ...
      {show && <History open={show} handleOpen={handleOpen} />}
      {IPv6show && <IPv6 open={IPv6show} handleOpen={handleIPv6Open} />}
    </div>
  );
};
