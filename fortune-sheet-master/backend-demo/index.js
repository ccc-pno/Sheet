...
async function initMongoDB() {
  ...
  await RecordClient.db("admin").command({ ping: 1 });
}
initMongoDB();
...
async function getData(tableName) {
  ...
  if (!tableName) tableName = collectionName;
  // const typeFlag = type ? RecordCollectionName : collectionName;
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", async (data) => {
    ...
    if (msg.req === "getData") {
      ...
      await record(
        client.db(dbName).collection(RecordCollectionName),
        msg.data
      ); // record operation
